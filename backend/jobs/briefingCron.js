...
async function generateDigest(user, breaches, newsItems) {
  ...
  const prompt = `You are writing a weekly security digest email for a user of Byronaire Security, a personal cybersecurity toolkit.

Breach status for this user: ${breachStatus}
Breach Monitor (automated monitoring): ${user.monitoring?.enabled ? 'enabled' : 'not enabled'}
${newsContext}
...
<table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:10px;overflow:hidden;max-width:600px;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
  <tr><td style="background:#050505;padding:24px 32px;">
    <p style="color:#e23048;margin:0;font-size:20px;font-weight:800;letter-spacing:-0.01em;">Byronaire Security</p>
    <p style="color:#e2c78a;margin:4px 0 0;font-size:11px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;">Weekly Security Briefing</p>
  </td></tr>
  <tr><td style="padding:28px 32px 4px;">
    ...
    <div style="margin-bottom:22px;">
      <p style="color:#e23048;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;margin:0 0 8px;">Breach Status</p>
      <p style="color:#333;font-size:14px;line-height:1.65;margin:0;">${safe.breach}</p>
    </div>

    <div style="margin-bottom:22px;padding:16px;background:#fdf6f6;border-radius:6px;border-left:3px solid #e23048;">
      <p style="color:#e23048;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;margin:0 0 8px;">Security News</p>
      <p style="color:#333;font-size:14px;line-height:1.65;margin:0;">${safe.news}</p>
    </div>

    <div style="margin-bottom:28px;padding:16px;background:#fbf8f0;border-radius:6px;border-left:3px solid #e2c78a;">
      <p style="color:#9c1220;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;margin:0 0 8px;">This Week&apos;s Action</p>
  ...
  <tr><td style="background:#f8f8f8;padding:16px 32px;border-top:1px solid #eee;">
    <p style="color:#aaa;font-size:11px;margin:0 0 4px;">
      <a href="${APP_URL}/briefing" style="color:#e23048;text-decoration:none;">Manage briefing settings</a>
      &nbsp;&middot;&nbsp;
      <a href="${APP_URL}/account" style="color:#e23048;text-decoration:none;">Privacy dashboard</a>
    </p>
    <p style="color:#ccc;font-size:10px;margin:0;">AI-generated by Anthropic Haiku &middot; May contain inaccuracies &middot; Not professional security advice</p>
  </td></tr>
</table>
</td></tr>
</table>
  ...
  for (const user of subscribers) {
    try {
      ...
      try {
        ...
        continue; // AI call failed — skip this user this week
      }
      ...
      const subject = breachCount > 0
        ? `Byronaire Security Briefing — ${breachCount} breach${breachCount > 1 ? 'es' : ''} detected`
        : 'Byronaire Security Briefing — Your weekly security update';

      await sendMail({
        to:      user.email,
        subject,
        text:    [digest.breachSection, digest.newsSection, `This week's action: ${digest.actionSection}`, '', '— Byronaire Security AI Briefing'].join('\n\n'),
        html,
      });
    } catch {
